...
// Non-breaking space between an amount and a currency code.
...
export function is12h( wpTimeFormat ) {
	return (
		/[gh]/.test( String( wpTimeFormat || '' ) ) ||
		/[aA]/.test( String( wpTimeFormat || '' ) )
	);
}
...
export function formatTime(
	value,
	{ locale = 'en', timeFormat = 'H:i' } = {}
) {
	const text = String( value || '' );
	const date = /^\d{2}:\d{2}/.test( text )
		? parseLocal( '2000-01-01 ' + text )
		: parseLocal( text );
	if ( ! date ) {
		return text;
	}
...
export function formatMoney(
	amount,
	{ currency = 'USD', symbol = '', position = 'before', locale = 'en' } = {}
) {
	...
	if ( position === 'after' ) {
		return `${ number }${ NBSP }${ sign }`;
	}
	return /^[A-Z]{3}$/.test( sign )
		? `${ sign }${ NBSP }${ number }`
		: `${ sign }${ number }`;
}
...
export function formatDuration( minutes, labels = { h: 'h', min: 'min' } ) {
	...
	return rest
		? `${ hours } ${ labels.h } ${ rest } ${ labels.min }`
		: `${ hours } ${ labels.h }`;
}
