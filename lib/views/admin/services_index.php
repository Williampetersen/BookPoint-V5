...
<div class="wrap">
  <h1>
    <?php echo esc_html__('Services', 'bookpoint-v5'); ?>
    <a href="<?php echo esc_url(admin_url('admin.php?page=pointlybooking_services_edit')); ?>" class="page-title-action">
      <?php echo esc_html__('Add New', 'bookpoint-v5'); ?>
    </a>
  </h1>

  <?php if (!empty($updated_notice)) : ?>
    <div class="notice notice-success is-dismissible"><p><?php echo esc_html__('Service saved.', 'bookpoint-v5'); ?></p></div>
  <?php endif; ?>
  <?php if (!empty($deleted_notice)) : ?>
    <div class="notice notice-success is-dismissible"><p><?php echo esc_html__('Service deleted.', 'bookpoint-v5'); ?></p></div>
  <?php endif; ?>
  ...
  <table class="widefat striped">
    <thead>
      <tr>
        ...
        <th><?php echo esc_html__('Duration', 'bookpoint-v5'); ?></th>
        <th><?php echo esc_html__('Price', 'bookpoint-v5'); ?></th>
        <th><?php echo esc_html__('Active', 'bookpoint-v5'); ?></th>
        <th><?php echo esc_html__('Actions', 'bookpoint-v5'); ?></th>
      </tr>
    </thead>
    <tbody>
      <?php if (empty($services)) : ?>
        <tr><td colspan="7"><?php echo esc_html__('No services yet.', 'bookpoint-v5'); ?></td></tr>
      <?php else : ?>
        <?php foreach ($services as $s) : ?>
          ...
          <tr>
            ...
            <td><?php echo esc_html((int)$s['duration_minutes']); ?> <?php echo esc_html__('min', 'bookpoint-v5'); ?></td>
            <td><?php echo esc_html($s['currency']); ?> <?php echo esc_html(number_format_i18n(((int)$s['price_cents']) / 100, 2)); ?></td>
            <td><?php echo esc_html((int)$s['is_active'] === 1 ? __('Yes', 'bookpoint-v5') : __('No', 'bookpoint-v5')); ?></td>
            <td>
              ...
              <a href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=pointlybooking_services_delete&id=' . absint($s['id'])), 'pointlybooking_admin')); ?>"
                 onclick="return confirm('<?php echo esc_js(__('Delete this service?', 'bookpoint-v5')); ?>');">
