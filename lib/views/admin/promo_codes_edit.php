...
$pointlybooking_actions_html = '<a class="bp-top-btn" href="' . esc_url(admin_url('admin.php?page=pointlybooking_promo_codes')) . '">' . esc_html__('Back to Promo Codes', 'bookpoint-v5') . '</a>';
pointlybooking_render_legacy_shell_start(
  $id ? esc_html__('Edit Promo Code', 'bookpoint-v5') : esc_html__('Add Promo Code', 'bookpoint-v5'),
  esc_html__('Create discounts and manage usage limits.', 'bookpoint-v5'),
...
?>
  ...
  <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
    ...
    <table class="form-table">
      <tr>
        <th><label><?php echo esc_html__('Code', 'bookpoint-v5'); ?></label></th>
        <td><input type="text" name="code" value="<?php echo esc_attr($code); ?>" class="regular-text" required placeholder="SAVE10"></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Type', 'bookpoint-v5'); ?></label></th>
        <td>
          <select name="type">
            <option value="percent" <?php selected($type, 'percent'); ?>><?php echo esc_html__('Percent', 'bookpoint-v5'); ?></option>
            <option value="fixed" <?php selected($type, 'fixed'); ?>><?php echo esc_html__('Fixed', 'bookpoint-v5'); ?></option>
          </select>
        </td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Amount', 'bookpoint-v5'); ?></label></th>
        <td><input type="number" step="0.01" name="amount" value="<?php echo esc_attr($amount); ?>" class="small-text" required></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Starts At', 'bookpoint-v5'); ?></label></th>
        <td><input type="datetime-local" name="starts_at" value="<?php echo esc_attr($starts_at ? gmdate('Y-m-d\TH:i', strtotime($starts_at)) : ''); ?>"></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Ends At', 'bookpoint-v5'); ?></label></th>
        <td><input type="datetime-local" name="ends_at" value="<?php echo esc_attr($ends_at ? gmdate('Y-m-d\TH:i', strtotime($ends_at)) : ''); ?>"></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Max Uses', 'bookpoint-v5'); ?></label></th>
        <td><input type="number" name="max_uses" value="<?php echo esc_attr($max_uses); ?>" class="small-text" placeholder="(optional)"></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Min Total', 'bookpoint-v5'); ?></label></th>
    ...
    <p class="submit">
      <button type="submit" class="bp-btn bp-btn-primary">
        <?php echo esc_html($id ? __('Save Changes', 'bookpoint-v5') : __('Create Promo Code', 'bookpoint-v5')); ?>
      </button>
      ...
      <?php if ($id): ?>
        <a class="bp-btn bp-btn-danger" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=pointlybooking_promo_codes_delete&id=' . absint($id)), 'pointlybooking_admin')); ?>" onclick="return confirm('<?php echo esc_js(__('Delete promo code?', 'bookpoint-v5')); ?>');">
