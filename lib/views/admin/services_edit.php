...
?>
<?php
$pointlybooking_actions_html = '<a class="bp-top-btn" href="' . esc_url(admin_url('admin.php?page=pointlybooking_services')) . '">' . esc_html__('Back to Services', 'bookpoint-v5') . '</a>';
pointlybooking_render_legacy_shell_start(
  $id ? esc_html__('Edit Service', 'bookpoint-v5') : esc_html__('Add Service', 'bookpoint-v5'),
  esc_html__('Manage service details, pricing, and availability.', 'bookpoint-v5'),
...
?>
  ...
  <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
    ...
    <table class="form-table" role="presentation">
      <tr>
        <th><label for="pointlybooking_name"><?php echo esc_html__('Name', 'bookpoint-v5'); ?></label></th>
        <td>
      ...
      <tr>
        <th><label for="pointlybooking_duration"><?php echo esc_html__('Duration (minutes)', 'bookpoint-v5'); ?></label></th>
        <td>
      ...
      <tr>
        <th><label for="pointlybooking_price"><?php echo esc_html__('Price (cents)', 'bookpoint-v5'); ?></label></th>
        <td>
          <input type="number" id="pointlybooking_price" name="price_cents" min="0" value="<?php echo esc_attr($price_cents); ?>">
          <p class="description"><?php echo esc_html__('Example: 2500 = 25.00', 'bookpoint-v5'); ?></p>
          <?php pointlybooking_field_error($errors, 'price_cents'); ?>
        </td>
      </tr>

      <tr>
        <th><label for="pointlybooking_currency"><?php echo esc_html__('Currency', 'bookpoint-v5'); ?></label></th>
        <td>
      ...
      <tr>
        <th><?php echo esc_html__('Description', 'bookpoint-v5'); ?></th>
        <td>
      ...
      <tr>
        <th><label><?php echo esc_html__('Categories', 'bookpoint-v5'); ?></label></th>
        <td>
          ...
          <p class="description"><?php echo esc_html__('A service can belong to multiple categories.', 'bookpoint-v5'); ?></p>
        </td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Service Image', 'bookpoint-v5'); ?></label></th>
        <td>
          ...
          <button type="button" class="button" id="pointlybooking_service_pick_image"><?php echo esc_html__('Choose Image', 'bookpoint-v5'); ?></button>
          <button type="button" class="button" id="pointlybooking_service_remove_image"><?php echo esc_html__('Remove', 'bookpoint-v5'); ?></button>

          <p class="description"><?php echo esc_html__('Stored as Media Library attachment ID.', 'bookpoint-v5'); ?></p>
        </td>
      </tr>

      <tr>
        <th><?php echo esc_html__('Active', 'bookpoint-v5'); ?></th>
        <td>
          <label>
            <input type="checkbox" name="is_active" value="1" <?php checked($is_active, 1); ?>>
            <?php echo esc_html__('Service is active', 'bookpoint-v5'); ?>
          </label>
        </td>
      </tr>

      <tr>
        <th><label for="pointlybooking_capacity"><?php echo esc_html__('Capacity', 'bookpoint-v5'); ?></label></th>
        <td>
          ...
          <p class="description"><?php echo esc_html__('How many bookings can be made for the same time slot.', 'bookpoint-v5'); ?></p>
        </td>
      </tr>

      <tr>
        <th><label for="pointlybooking_buf_before"><?php echo esc_html__('Buffer before (minutes)', 'bookpoint-v5'); ?></label></th>
        <td>
      ...
      <tr>
        <th><label for="pointlybooking_buf_after"><?php echo esc_html__('Buffer after (minutes)', 'bookpoint-v5'); ?></label></th>
        <td>
      ...
      <tr>
        <th><?php echo esc_html__('Use Global Schedule', 'bookpoint-v5'); ?></th>
        <td>
          <label>
            <input type="checkbox" name="use_global_schedule" value="1" <?php checked($use_global_schedule, 1); ?>>
            <?php echo esc_html__('Use global weekly schedule from Settings', 'bookpoint-v5'); ?>
          </label>
          <p class="description"><?php echo esc_html__('If disabled, you can provide a service-specific schedule JSON below.', 'bookpoint-v5'); ?></p>
        </td>
      </tr>

      <tr>
        <th><?php echo esc_html__('Service Schedule JSON', 'bookpoint-v5'); ?></th>
        <td>
          ...
          <p class="description"><?php echo esc_html__('Optional. Keys are weekday numbers 0-6. Values are "HH:MM-HH:MM" or empty for closed.', 'bookpoint-v5'); ?></p>
        </td>
      </tr>
    </table>

    <h2 style="margin:18px 0 10px;">
      <?php esc_html_e('Agents for this service', 'bookpoint-v5'); ?>
    </h2>
    ...
    <?php if (!empty($all_agents)) : ?>
      ...
      <?php endforeach; ?>
    <?php else : ?>
      <p class="bp-muted"><?php esc_html_e('No agents yet. Add agents first.', 'bookpoint-v5'); ?></p>
    <?php endif; ?>

    <p class="submit">
      <button type="submit" class="bp-btn bp-btn-primary"><?php echo esc_html__('Save Service', 'bookpoint-v5'); ?></button>
      <a class="bp-btn" href="<?php echo esc_url(admin_url('admin.php?page=pointlybooking_services')); ?>"><?php echo esc_html__('Back', 'bookpoint-v5'); ?></a>
      <?php if ($id): ?>
        <a class="bp-btn bp-btn-danger" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=pointlybooking_services_delete&id=' . absint($id)), 'pointlybooking_admin')); ?>" onclick="return confirm('<?php echo esc_js(__('Delete service?', 'bookpoint-v5')); ?>');">
