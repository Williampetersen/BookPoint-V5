(function($){
  function renderPreview(url){
    var $preview = $('#bp_extra_image_preview');
    if(!url){
  ...
  $(document).on('click', '#bp_extra_pick_image', function(e){
    e.preventDefault();
    var frame = wp.media({
    ...
    frame.on('select', function(){
      ...
      $('#bp_extra_image_id').val(attachment.id);
      renderPreview(attachment.sizes && attachment.sizes.medium ? attachment.sizes.medium.url : attachment.url);
    });
    frame.open();
  });

  $(document).on('click', '#bp_extra_remove_image', function(e){
    e.preventDefault();
    $('#bp_extra_image_id').val('');
    renderPreview('');
  });
})(jQuery);
