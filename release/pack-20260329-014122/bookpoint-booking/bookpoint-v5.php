...
if (!function_exists('pointlybooking_request_has_key')) {
  function pointlybooking_request_has_key(string $method, string $key): bool {
    ...
    if ($method === 'get') {
      // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing -- Centralized helper for request-key existence checks; nonce/capability checks are enforced in action handlers.
...
if (!function_exists('pointlybooking_request_value_is_array')) {
  function pointlybooking_request_value_is_array(string $method, string $key): bool {
    if ($method === 'get') {
      ...
      // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing -- Centralized helper inspects request value type before sanitization.
      return is_array(wp_unslash($_GET[$key]));
    }
    ...
    if ($method === 'post') {
      ...
      // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing -- Centralized helper inspects request value type before sanitization.
      return is_array(wp_unslash($_POST[$key]));
    }
    ...
    if ($method === 'server') {
      ...
      return is_array(wp_unslash($_SERVER[$key]));
    }
...
if (!function_exists('pointlybooking_request_array')) {
  function pointlybooking_request_array(string $method, string $key): array {
    ...
    if ($method === 'get') {
      ...
      // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing -- Centralized helper reads request arrays; nonce/capability checks are performed by calling actions.
      // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- This helper intentionally returns raw unslashed array values for context-specific per-field sanitization at call sites.
      ...
      // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- This helper intentionally returns raw unslashed server values for context-specific sanitization at call sites.
...
if (!function_exists('pointlybooking_request_scalar')) {
  function pointlybooking_request_scalar(string $method, string $key): string {
    ...
    if ($method === 'get') {
      ...
      // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing -- Centralized helper reads request scalars; nonce/capability checks are handled in action handlers.
      // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- This helper intentionally returns raw unslashed scalar values so each caller can apply context-specific sanitization.
      $value = wp_unslash($_GET[$key]);
    } elseif ($method === 'post') {
      if (!isset($_POST[$key])) {
        return '';
...
if ($pointlybooking_existing_class !== '') {
  ...
  if (!$pointlybooking_same_file) {
    ...
    if ($pointlybooking_existing_file !== '') {
      ...
      $pointlybooking_loaded_from = __('Loaded from: %s', 'bookpoint-booking');
    ...
    add_action('admin_notices', function () use ($pointlybooking_product, $pointlybooking_details) {
      ...
      $message_template = __('%1$s could not be loaded because another copy of BookPoint is already active. Please deactivate the other BookPoint plugin first, then activate %2$s.%3$s', 'bookpoint-booking');
      $message = sprintf(
        $message_template,
