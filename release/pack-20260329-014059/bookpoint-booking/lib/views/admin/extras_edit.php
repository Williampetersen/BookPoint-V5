...
$pointlybooking_actions_html = '<a class="bp-top-btn" href="' . esc_url(admin_url('admin.php?page=pointlybooking_extras')) . '">' . esc_html__('Back to Extras', 'bookpoint-booking') . '</a>';
pointlybooking_render_legacy_shell_start(
  $id ? esc_html__('Edit Extra', 'bookpoint-booking') : esc_html__('Add Extra', 'bookpoint-booking'),
  esc_html__('Configure extra items, pricing, and service availability.', 'bookpoint-booking'),
  ...
  <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
    ...
    <table class="form-table">
      <tr>
        ...
        <td>
          ...
          <p class="description"><?php echo esc_html__('This extra will appear for selected services.', 'bookpoint-booking'); ?></p>
        </td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Name', 'bookpoint-booking'); ?></label></th>
        <td><input type="text" name="name" value="<?php echo esc_attr($name); ?>" class="regular-text" required></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Description', 'bookpoint-booking'); ?></label></th>
        <td><textarea name="description" class="large-text" rows="4"><?php echo esc_textarea($description); ?></textarea></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Price', 'bookpoint-booking'); ?></label></th>
        <td><input type="number" step="0.01" name="price" value="<?php echo esc_attr($price); ?>" class="small-text"></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Duration (minutes)', 'bookpoint-booking'); ?></label></th>
        <td><input type="number" name="duration_min" value="<?php echo esc_attr($duration_min); ?>" class="small-text"></td>
      </tr>

      <tr>
        <th><label><?php echo esc_html__('Image', 'bookpoint-booking'); ?></label></th>
        <td>
          ...
          <button type="button" class="button" id="pointlybooking_extra_pick_image"><?php echo esc_html__('Choose Image', 'bookpoint-booking'); ?></button>
          <button type="button" class="button" id="pointlybooking_extra_remove_image"><?php echo esc_html__('Remove', 'bookpoint-booking'); ?></button>
        </td>
      </tr>
    ...
    <p class="submit">
      <button type="submit" class="bp-btn bp-btn-primary">
        <?php echo esc_html($id ? __('Save Changes', 'bookpoint-booking') : __('Create Extra', 'bookpoint-booking')); ?>
      </button>
      ...
      <?php if ($id): ?>
        <a class="bp-btn bp-btn-danger" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=pointlybooking_extras_delete&id=' . absint($id)), 'pointlybooking_admin')); ?>" onclick="return confirm('<?php echo esc_js(__('Delete extra?', 'bookpoint-booking')); ?>');">
