...
if (!function_exists('pointlybooking_verify_admin_nonce')) {
  function pointlybooking_verify_admin_nonce(string $action, string $field = '_wpnonce'): bool {
    ...
    if (is_array($_POST) && array_key_exists($field, $_POST)) {
      ...
      if (!is_array($post_value) && !is_object($post_value)) {
        $nonce = sanitize_text_field((string) $post_value);
      }
    }
    if ($nonce === '' && is_array($_GET) && array_key_exists($field, $_GET)) {
      $get_value = wp_unslash($_GET[$field]);
      if (!is_array($get_value) && !is_object($get_value)) {
        $nonce = sanitize_text_field((string) $get_value);
      }
    }
    if ($nonce === '') {
      return false;
