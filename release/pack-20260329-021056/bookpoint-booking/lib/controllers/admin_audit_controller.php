...
final class POINTLYBOOKING_AdminAuditController extends POINTLYBOOKING_Controller {
  ...
  public function index() : void {
    ...
    $event = $has_filter_nonce ? $this->query_key('event') : '';
    if ($event !== '' && !in_array($event, $events, true)) {
      $event = '';
    }

    $actor_type = $has_filter_nonce ? $this->query_key('actor_type') : '';
    if (!in_array($actor_type, ['', 'admin', 'customer', 'system'], true)) {
      $actor_type = '';
    }

    $date_from = $has_filter_nonce ? $this->sanitize_ymd($this->query_text('date_from')) : '';
    $date_to = $has_filter_nonce ? $this->sanitize_ymd($this->query_text('date_to')) : '';
    ...
    $filters = [
      ...
      'booking_id' => $has_filter_nonce ? $this->query_absint('booking_id') : 0,
      'customer_id' => $has_filter_nonce ? $this->query_absint('customer_id') : 0,
      'date_from' => $date_from,
      'date_to' => $date_to,
    ];

    $page = $has_filter_nonce ? max(1, $this->query_absint('paged')) : 1;
    $per_page = $has_filter_nonce ? max(1, min(200, $this->query_absint('per_page'))) : 50;
    if ($per_page < 1) {
      $per_page = 50;
    }
