  ...
  if (is_array($extras_raw) && !empty($extras_raw)) {
    ...
    if (!empty($ids)) {
      ...
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- table name is generated from hardcoded suffix via pointlybooking_table(); IN clause placeholders are built dynamically from sanitized integer IDs.
          "SELECT id, name, price FROM {$t_extras} WHERE id IN ({$in})",
          $ids
        ),
        ARRAY_A
      ) ?: [];
...
function pointlybooking_rest_admin_agents_list(WP_REST_Request $req) {
  ...
  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- no user input; selected columns and order are derived from schema checks.
