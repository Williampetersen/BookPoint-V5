...
function pointlybooking_admin_calendar_get_bookings(WP_REST_Request $req) {
  ...
  $rows = $wpdb->get_results(
    $wpdb->prepare(
      // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table names are generated from hardcoded suffixes via pointlybooking_table(); $select_sql and $order_by are built from whitelisted column names; $where contains only hardcoded SQL fragments with placeholders.
      "SELECT {$select_sql}
      FROM {$tB} b
