...
function pointlybooking_rest_admin_services_list(WP_REST_Request $req) {
  ...
  $rows = $wpdb->get_results(
    $wpdb->prepare(
      // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from hardcoded suffix via pointlybooking_table(); $where contains only hardcoded SQL fragments with placeholders; $order is built from hardcoded column names.
      "SELECT * FROM {$t} WHERE {$where} ORDER BY {$order}",
      $params
    ),
    ARRAY_A
  ) ?: [];
...
function pointlybooking_rest_admin_availability_slots(WP_REST_Request $req) {
  ...
  $make_slots_for_agent = function(int $aId) use ($wpdb, $date, $occupied, $capacity, $exclude_id) {
    ...
    if ($exclude_id > 0) {
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table names are generated from hardcoded suffixes via pointlybooking_table(); duration/buffer expressions are selected from a strict column whitelist.
          "SELECT
