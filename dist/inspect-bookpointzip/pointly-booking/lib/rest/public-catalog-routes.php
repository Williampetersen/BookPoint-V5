  ...
  if ($category_id > 0) {
    ...
    if ($rel_exists) {
      if ($has_is_active) {
        $rows = $wpdb->get_results(
          $wpdb->prepare(
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table names are generated from $wpdb->prefix + hardcoded suffix.
            "SELECT s.*
            FROM {$t} s
    ...
    if (empty($rows) && $has_category_col) {
      if ($has_is_active) {
        $rows = $wpdb->get_results(
          $wpdb->prepare(
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from $wpdb->prefix + hardcoded suffix.
            "SELECT *
            FROM {$t}
...
function pointlybooking_rest_public_extras(WP_REST_Request $req) {
  ...
  $rows = $wpdb->get_results(
    $wpdb->prepare(
      // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table names are generated from hardcoded suffixes via pointlybooking_table(); $select is built from a strict column whitelist.
      "SELECT {$select}
      FROM {$t} e
