...
if ($pointlybooking_existing_class !== '') {
  ...
  if (!$pointlybooking_same_file) {
    ...
    if ($pointlybooking_existing_file !== '') {
      ...
      $pointlybooking_details = "\n\n" . sprintf(__('Loaded from: %s', 'pointly-booking'), $pointlybooking_existing_display);
    }
    ...
    add_action('admin_notices', function () use ($pointlybooking_product, $pointlybooking_details) {
      ...
      $message = sprintf(
        ...
        __('%1$s could not be loaded because another copy of BookPoint is already active. Please deactivate the other BookPoint plugin first, then activate %2$s.%3$s', 'pointly-booking'),
...
final class POINTLYBOOKING_Core_Plugin {
  ...
  private static function define_constants() : void {
    if (!defined('POINTLY_BOOKING_PLUGIN_FILE')) define('POINTLY_BOOKING_PLUGIN_FILE', __FILE__);
    if (!defined('POINTLY_BOOKING_PLUGIN_DIR')) define('POINTLY_BOOKING_PLUGIN_DIR', plugin_dir_path(__FILE__));
    if (!defined('POINTLY_BOOKING_PLUGIN_URL')) define('POINTLY_BOOKING_PLUGIN_URL', plugin_dir_url(__FILE__));

    if (!defined('POINTLYBOOKING_PLUGIN_FILE')) define('POINTLYBOOKING_PLUGIN_FILE', POINTLY_BOOKING_PLUGIN_FILE);
    if (!defined('POINTLYBOOKING_PLUGIN_PATH')) define('POINTLYBOOKING_PLUGIN_PATH', POINTLY_BOOKING_PLUGIN_DIR);
    if (!defined('POINTLYBOOKING_PLUGIN_URL'))  define('POINTLYBOOKING_PLUGIN_URL', POINTLY_BOOKING_PLUGIN_URL);
  ...
  private static function add_index_if_missing(string $table, string $index, array $columns) : void {
    ...
    $table_sql = self::quote_sql_identifier($table);
    $index_sql = self::quote_sql_identifier($index);
    $columns_sql = implode(', ', array_map([__CLASS__, 'quote_sql_identifier'], $columns));

    $exists = $wpdb->get_var($wpdb->prepare("SHOW INDEX FROM {$table_sql} WHERE Key_name = %s", $index));
    if (!$exists) {
      $wpdb->query("ALTER TABLE {$table_sql} ADD INDEX {$index_sql} ({$columns_sql})");
    }
  }
  ...
  private static function seed_default_agent_hours() : void {
    ...
    $agents = $wpdb->get_results(
      $wpdb->prepare(
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table names are generated from hardcoded suffixes via pointlybooking_table().
        "SELECT id FROM {$t_agents} WHERE %d=%d",
        1,
        1
      ),
      ARRAY_A
    ...
    register_rest_route('pointly-booking/v1', '/admin/agents', [
      ...
      'permission_callback' => [__CLASS__, 'rest_can_manage_agents'],
        ...
        $rows = $wpdb->get_results(
          $wpdb->prepare(
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from hardcoded suffix via pointlybooking_table().
            "SELECT * FROM {$tA} WHERE %d=%d ORDER BY id DESC",
            1,
            1
          ),
          ARRAY_A
    ...
    register_rest_route('pointly-booking/v1', '/admin/bookings', [
      ...
      'callback' => function(\WP_REST_Request $req){
        ...
        $items = $wpdb->get_results(
          $wpdb->prepare(
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table names are generated from hardcoded suffixes via pointlybooking_table(); $order is a strict ASC/DESC whitelist; $where contains only hardcoded SQL fragments with placeholders.
            "SELECT
               b.id,
               b.start_datetime,
               b.end_datetime,
      ...
      'callback' => function(\WP_REST_Request $req){
        ...
        if ($has_form_fields) {
          $defs = $wpdb->get_results(
            $wpdb->prepare(
              // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from hardcoded suffix via pointlybooking_table().
              "SELECT * FROM {$tFields} ORDER BY sort_order ASC, id ASC LIMIT %d",
              100
            ),
            ARRAY_A
          ) ?: [];
  ...
  public static function handle_customers_export_csv() : void {
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from hardcoded suffix via pointlybooking_table().
        "SELECT * FROM {$table} WHERE %d=%d ORDER BY id DESC",
        1,
        1
      ),
      ARRAY_A
  ...
  public static function shortcode_customer_portal() : string {
    ...
    if (in_array($step, ['verify', 'list'], true)) {
      $valid_view_nonce = ($email !== '') && wp_verify_nonce($view_nonce, self::portal_view_nonce_action($step, $email));
      if (!$valid_view_nonce) {
        $step = 'email';
        $email = '';
        $session = '';
  ...
  function pointlybooking_rest_get_services(\WP_REST_Request $req) {
    ...
    if ($category_id > 0) {
      ...
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from hardcoded suffix via pointlybooking_table().
          "SELECT id, name, price_cents, duration_minutes, category_id, image_id FROM {$t} WHERE is_active = %d ORDER BY id DESC LIMIT 500",
          1
        ),
        ARRAY_A
      );
...
if (!function_exists('pointlybooking_rest_create_booking')) {
  function pointlybooking_rest_create_booking(\WP_REST_Request $req) {
    ...
    $svc = $wpdb->get_row(
      $wpdb->prepare(
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from hardcoded suffix via pointlybooking_table().
        "SELECT id, price_cents, duration_minutes, category_id FROM {$t_services} WHERE id = %d AND is_active = 1",
        $service_id
      ),
      ARRAY_A
    );
    ...
    $valid_extras = $wpdb->get_results(
      $wpdb->prepare(
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name is generated from hardcoded suffix via pointlybooking_table().
        "SELECT id, price FROM {$t_extras} WHERE service_id=%d AND is_active=1",
        $service_id
      ),
      ARRAY_A
    );
