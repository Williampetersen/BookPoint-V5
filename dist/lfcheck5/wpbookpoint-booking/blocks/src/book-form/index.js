...
registerBlockType(metadata.name, {
  ...
  edit: ({ attributes, setAttributes }) => {
    ...
    const options = useMemo(() => {
      const base = [{ label: __('Select a serviceâ€¦', 'bookpoint'), value: '0' }];
    ...
    return (
      <>
          ...
          <PanelBody title={__('BookPoint Settings', 'bookpoint')} initialOpen={true}>
            ...
            {loading && (
              <Notice status="info" isDismissible={false}>
                {__('Loading servicesâ€¦', 'bookpoint')}
              </Notice>
            )}
            ...
            <SelectControl
              ...
              help={__('Services are loaded from BookPoint â†’ Services.', 'bookpoint')}
            />
        ...
        <div style={{ padding: '14px', border: '1px solid #ddd', borderRadius: '6px' }}>
          <strong>{__('BookPoint â€“ Booking Form', 'bookpoint')}</strong>
