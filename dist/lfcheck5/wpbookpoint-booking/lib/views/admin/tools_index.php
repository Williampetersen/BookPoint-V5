...
<?php pointlybooking_render_legacy_shell_start(esc_html__('Tools', 'bookpoint'), esc_html__('Maintenance, tests, and data utilities.', 'bookpoint'), '', 'tools'); ?>
    ...
    <tbody>
      <?php foreach (($exists ?? []) as $t => $ok) : ?>
        <tr>
          ...
          <td><?php echo $ok ? 'Ã¢Å“â€¦ OK' : 'Ã¢ÂÅ’ Missing'; ?></td>
        </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
  ...
  <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
    ...
    <p class="description"><?php esc_html_e('Make sure webhooks are enabled and URL is set in Settings Ã¢â€ â€™ Webhooks.', 'bookpoint'); ?></p>
  </form>
