...
add_filter('admin_body_class', function($classes){
  ...
  // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only page routing context.
...
final class POINTLYBOOKING_Core_Plugin {
  ...
  private static function register_hooks() : void {
    ...
    add_action('admin_init', function () {
      if (!is_admin()) return;
      // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- internal admin route remapping.
      $page = sanitize_key(wp_unslash($_GET['page'] ?? ''));
      if ($page === '') return;
  ...
  private static function is_menu_debug_enabled(): bool {
    ...
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- debug flag only.
    return sanitize_text_field((string) wp_unslash($_GET['pointlybooking_menu_debug'] ?? '')) === '1';
  }
  ...
  public static function enqueue_admin_assets(string $hook): void {
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- admin page switch, read-only.
    $page = sanitize_key(wp_unslash($_GET['page'] ?? ''));
    if ($page === '') return;
