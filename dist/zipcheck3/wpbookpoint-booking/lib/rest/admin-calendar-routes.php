...
function pointlybooking_admin_calendar_get_bookings(WP_REST_Request $req) {
  ...
  $tC = $wpdb->prefix . 'pointlybooking_customers';
  ...
  foreach ($rows as $r) {
    ...
    $data[] = [
      ...
      'title' => trim((($r['service_name'] ?? 'Service') ?: 'Service') . ' â€¢ ' . ($customer_name ?: 'Customer')),
    ];
  }
...
function pointlybooking_rest_admin_calendar_events(WP_REST_Request $req) {
  ...
  $t_agent= $wpdb->prefix . 'pointlybooking_agents';
  ...
  foreach ($rows as $r) {
    ...
    $title = trim((($r['service_name'] ?? 'Service') ?: 'Service') . ' â€¢ ' . ($customer_name ?: ('#'.$r['id'])));
