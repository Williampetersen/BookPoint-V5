...
final class POINTLYBOOKING_PublicBookingsController extends POINTLYBOOKING_Controller {
  ...
  private function verify_public_nonce() : bool {
    ...
    return wp_verify_nonce($nonce, 'pointlybooking_public');
  }

  public function slots() : void {
    if (!$this->verify_public_nonce()) {
      $this->json_error('pointlybooking_BAD_NONCE', __('Security check failed.', 'bookpoint'));
    }
    ...
    if ($service_id <= 0 || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) {
      $this->json_error('pointlybooking_INVALID_PARAMS', __('Invalid parameters.', 'bookpoint'));
    }

    // Step 14: Validate date is within booking window
    if (!POINTLYBOOKING_ScheduleHelper::is_date_allowed($date)) {
      $this->json_error('pointlybooking_DATE_OUT_OF_RANGE', __('Booking date is outside allowed range.', 'bookpoint'));
    }

    // Get service first for step 15 (service-based schedule)
    $service = POINTLYBOOKING_ServiceModel::find($service_id);
    if (!$service) {
      $this->json_error('pointlybooking_SERVICE_NOT_FOUND', __('Service not found.', 'bookpoint'));
    }
  ...
  public function submit() : void {
    ...
    $hp = (string) wp_unslash($_POST['pointlybooking_hp'] ?? '');
    if ($hp !== '') {
      $this->json_error('pointlybooking_SPAM', __('Spam detected.', 'bookpoint'));
    }
    ...
    if ($service_id <= 0) {
      $this->json_error('pointlybooking_INVALID_SERVICE', __('Invalid service.', 'bookpoint'));
    }

    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date) || !preg_match('/^\d{2}:\d{2}$/', $time)) {
      $this->json_error('pointlybooking_INVALID_DATE_TIME', __('Invalid date or time.', 'bookpoint'));
    }
    ...
    if (!POINTLYBOOKING_AvailabilityHelper::is_slot_available($service_id, $start_dt_adj, $end_dt_adj, $capacity, $agent_id)) {
      $this->json_error('pointlybooking_NOT_AVAILABLE', __('This time is no longer available. Please choose another slot.', 'bookpoint'));
    }
    ...
    if ($booking_id <= 0) {
      $this->json_error('pointlybooking_CREATE_FAILED', __('Could not create booking.', 'bookpoint'));
    }
  ...
  public function render_manage_page() : void {
    ...
    wp_enqueue_style('bp-portal', POINTLYBOOKING_PLUGIN_URL . 'public/stylesheets/portal.css', [], BPV5_BookPoint_Core_Plugin::VERSION);
    wp_enqueue_script('bp-manage', POINTLYBOOKING_PLUGIN_URL . 'public/javascripts/manage-booking.js', [], BPV5_BookPoint_Core_Plugin::VERSION, true);
