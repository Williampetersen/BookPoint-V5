...
final class POINTLYBOOKING_Core_Plugin {
  ...
  private static function load_textdomain() : void {
    // WordPress.org loads translations for the plugin slug automatically.
  }
  ...
  private static function add_column_if_missing(string $table, string $column, string $definition) : void {
    ...
    $exists = $wpdb->get_var($wpdb->prepare("SHOW COLUMNS FROM {$quoted_table} LIKE %s", $column));
    if (!$exists) {
      self::run_add_column_query($quoted_table, $column, $definition);
    }
  }
  ...
  private static function add_index_if_missing(string $table, string $index, array $columns) : void {
    ...
    $quoted_table = self::quote_sql_identifier($table);
    $exists = $wpdb->get_var($wpdb->prepare("SHOW INDEX FROM {$quoted_table} WHERE Key_name = %s", $index));
    if (!$exists) {
      self::run_add_index_query($quoted_table, $index, $columns);
    }
  }

  private static function run_add_column_query(string $table_sql, string $column, string $definition): void {
    global $wpdb;

    if ($column === 'image_id' && $definition === 'BIGINT UNSIGNED NOT NULL DEFAULT 0') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `image_id` BIGINT UNSIGNED NOT NULL DEFAULT 0");
      return;
    }
    if ($column === 'sort_order' && $definition === 'INT NOT NULL DEFAULT 0') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `sort_order` INT NOT NULL DEFAULT 0");
      return;
    }
    if ($column === 'agent_id' && $definition === 'BIGINT UNSIGNED NULL') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `agent_id` BIGINT UNSIGNED NULL");
      return;
    }
    if ($column === 'is_recurring' && $definition === 'TINYINT NOT NULL DEFAULT 0') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `is_recurring` TINYINT NOT NULL DEFAULT 0");
      return;
    }
    if ($column === 'created_at' && $definition === 'DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP");
      return;
    }
    if ($column === 'updated_at' && $definition === 'DATETIME NULL') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `updated_at` DATETIME NULL");
      return;
    }
    if ($column === 'buffer_before' && $definition === 'INT NOT NULL DEFAULT 0') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `buffer_before` INT NOT NULL DEFAULT 0");
      return;
    }
    if ($column === 'buffer_after' && $definition === 'INT NOT NULL DEFAULT 0') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `buffer_after` INT NOT NULL DEFAULT 0");
      return;
    }
    if ($column === 'capacity' && $definition === 'INT NOT NULL DEFAULT 1') {
      $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN `capacity` INT NOT NULL DEFAULT 1");
    }
  }

  private static function run_add_index_query(string $table_sql, string $index, array $columns): void {
    global $wpdb;

    if ($index === 'agent_start_date' && $columns === ['agent_id', 'start_date']) {
      $wpdb->query("ALTER TABLE {$table_sql} ADD INDEX `agent_start_date` (`agent_id`, `start_date`)");
      return;
    }
    if ($index === 'service_start_date' && $columns === ['service_id', 'start_date']) {
      $wpdb->query("ALTER TABLE {$table_sql} ADD INDEX `service_start_date` (`service_id`, `start_date`)");
      return;
    }
    if ($index === 'sort_order' && $columns === ['sort_order']) {
      $wpdb->query("ALTER TABLE {$table_sql} ADD INDEX `sort_order` (`sort_order`)");
      return;
    }
    if ($index === 'agent_id' && $columns === ['agent_id']) {
      $wpdb->query("ALTER TABLE {$table_sql} ADD INDEX `agent_id` (`agent_id`)");
      return;
    }
    if ($index === 'agent_day' && $columns === ['agent_id', 'day_of_week']) {
      $wpdb->query("ALTER TABLE {$table_sql} ADD INDEX `agent_day` (`agent_id`, `day_of_week`)");
    }
  }
  ...
  private static function seed_default_agent_hours() : void {
    ...
    $quoted_agents = self::quote_sql_identifier($t_agents);
    $quoted_hours = self::quote_sql_identifier($t_hours);

    $agents = $wpdb->get_results(
      "SELECT id FROM {$quoted_agents} ORDER BY id ASC",
      ARRAY_A
    ) ?: [];
    foreach ($agents as $a) {
      $aid = (int)$a['id'];
      ...
      $exists = (int) $wpdb->get_var(
        $wpdb->prepare(
          "SELECT COUNT(*) FROM {$quoted_hours} WHERE agent_id=%d",
          $aid
        )
      );
  ...
  public static function register_rest_routes() : void {
    ...
    register_rest_route('pointly-booking/v1', '/admin/agents', [
      ...
      'permission_callback' => [__CLASS__, 'rest_can_manage_agents'],
        ...
        $quoted_agents = self::quote_sql_identifier($tA);
    ...
    register_rest_route('pointly-booking/v1', '/admin/bookings', [
      ...
      'callback' => function(\WP_REST_Request $req){
        ...
        $quoted_bookings = self::quote_sql_identifier($b);
        $quoted_customers = self::quote_sql_identifier($c);
        $quoted_services = self::quote_sql_identifier($s);
        $quoted_agents = self::quote_sql_identifier($a);
        ...
        $total = (int) $wpdb->get_var(
          $wpdb->prepare(
            "SELECT COUNT(*)
             ...
             LEFT JOIN {$quoted_customers} cust ON b.customer_id = cust.id
             LEFT JOIN {$quoted_services} srv ON b.service_id = srv.id
             LEFT JOIN {$quoted_agents} ag ON b.agent_id = ag.id
    ...
    register_rest_route('pointly-booking/v1', '/admin/bookings/(?P<id>\d+)', [
      ...
      'callback' => function(\WP_REST_Request $req){
        ...
        $quoted_services = self::quote_sql_identifier($s);
        $quoted_agents = self::quote_sql_identifier($a);
        $quoted_fields = self::quote_sql_identifier($tFields);
        ...
        if ($has_form_fields) {
          $defs = $wpdb->get_results(
            $wpdb->prepare(
              "SELECT * FROM {$quoted_fields} ORDER BY sort_order ASC, id ASC LIMIT %d",
              100
            ),
            ARRAY_A
          ) ?: [];
  ...
  public static function handle_customers_export_csv() : void {
    ...
    $quoted_table = pointlybooking_quote_sql_identifier($table);
    $rows = $wpdb->get_results(
      "SELECT * FROM {$quoted_table} ORDER BY id DESC",
      ARRAY_A
    ) ?: [];
...
if (!function_exists('pointlybooking_rest_get_services')) {
  function pointlybooking_rest_get_services(\WP_REST_Request $req) {
    ...
    $quoted_services = pointlybooking_quote_sql_identifier($t);
    $quoted_map = pointlybooking_quote_sql_identifier($map);
    ...
    if ($category_id > 0) {
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          "SELECT s.id, s.name, s.price_cents, s.duration_minutes, s.category_id, s.image_id
           ...
           INNER JOIN {$quoted_map} m ON m.service_id = s.id
      ...
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          "SELECT id, name, price_cents, duration_minutes, category_id, image_id FROM {$quoted_services} WHERE is_active = %d ORDER BY id DESC LIMIT 500",
          1
        ),
        ARRAY_A
      );
...
if (!function_exists('pointlybooking_rest_get_extras')) {
  function pointlybooking_rest_get_extras(\WP_REST_Request $req) {
    ...
    $quoted_extras = pointlybooking_quote_sql_identifier($extras_table);
    $quoted_map = pointlybooking_quote_sql_identifier($map);
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT e.id, e.name, e.price, e.duration_min, e.image_id, e.sort_order
         ...
         INNER JOIN {$quoted_map} m ON m.extra_id = e.id
...
if (!function_exists('pointlybooking_rest_get_agents')) {
  function pointlybooking_rest_get_agents(\WP_REST_Request $req) {
    ...
    $quoted_agents = pointlybooking_quote_sql_identifier($agents);
    $quoted_map = pointlybooking_quote_sql_identifier($map);
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT a.id, a.first_name, a.last_name, a.image_id
         ...
         INNER JOIN {$quoted_map} m ON m.agent_id = a.id
...
if (!function_exists('pointlybooking_rest_create_booking')) {
  function pointlybooking_rest_create_booking(\WP_REST_Request $req) {
    ...
    $quoted_services = pointlybooking_quote_sql_identifier($t_services);

    $svc = $wpdb->get_row(
      $wpdb->prepare(
        "SELECT id, price_cents, duration_minutes, category_id FROM {$quoted_services} WHERE id = %d AND is_active = 1",
        $service_id
      ),
      ARRAY_A
    );
    ...
    if ($agent_id > 0) {
      ...
      $ok = (int) $wpdb->get_var(
        $wpdb->prepare(
          "SELECT COUNT(*) FROM {$quoted_map} WHERE agent_id=%d AND service_id=%d",
          $agent_id,
          $service_id
        )
      );
    ...
    $quoted_extras = pointlybooking_quote_sql_identifier($t_extras);
    $valid_extras = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT id, price FROM {$quoted_extras} WHERE service_id=%d AND is_active=1",
        $service_id
      ),
      ARRAY_A
    );
