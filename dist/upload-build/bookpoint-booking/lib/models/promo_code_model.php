...
final class POINTLYBOOKING_PromoCodeModel {
  ...
  public static function find_by_code(string $code): ?array {
    ...
    $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$quoted_table} WHERE code=%s LIMIT 1", $code), ARRAY_A);
    return $row ?: null;
  }
  ...
  public static function increment_use(int $id): void {
    ...
    $quoted_table = self::quote_sql_identifier($t);
    $wpdb->query($wpdb->prepare("UPDATE {$quoted_table} SET uses_count = uses_count + 1 WHERE id=%d", $id));
  }
}
