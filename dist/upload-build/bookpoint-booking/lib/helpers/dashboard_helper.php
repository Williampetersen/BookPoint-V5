...
final class POINTLYBOOKING_DashboardHelper {
  ...
  public static function kpis_for_range(string $from, string $to): array {
    ...
    $quoted_bookings = self::quote_sql_identifier($table_bookings);

    $total = (int) $wpdb->get_var(
      $wpdb->prepare("SELECT COUNT(*) FROM {$quoted_bookings} WHERE DATE(created_at) BETWEEN %s AND %s", $from, $to)
    );

    $pending = (int) $wpdb->get_var(
      $wpdb->prepare("SELECT COUNT(*) FROM {$quoted_bookings} WHERE status = %s AND DATE(created_at) BETWEEN %s AND %s", 'pending', $from, $to)
    );

    $confirmed = (int) $wpdb->get_var(
      $wpdb->prepare("SELECT COUNT(*) FROM {$quoted_bookings} WHERE status = %s AND DATE(created_at) BETWEEN %s AND %s", 'confirmed', $from, $to)
    );

    $cancelled = (int) $wpdb->get_var(
      $wpdb->prepare("SELECT COUNT(*) FROM {$quoted_bookings} WHERE status = %s AND DATE(created_at) BETWEEN %s AND %s", 'cancelled', $from, $to)
    );

    $revenue = (float) $wpdb->get_var(
      $wpdb->prepare("SELECT COALESCE(SUM(total_price),0) FROM {$quoted_bookings} WHERE status = %s AND DATE(created_at) BETWEEN %s AND %s", 'confirmed', $from, $to)
    );
  ...
  public static function bookings_series_for_range(string $from, string $to): array {
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT DATE(created_at) as d, COUNT(*) as c FROM {$quoted_bookings} WHERE DATE(created_at) BETWEEN %s AND %s GROUP BY DATE(created_at) ORDER BY d ASC",
        $from,
        $to
      ),
      ARRAY_A
  ...
  public static function top_services(string $from, string $to, int $limit = 5): array {
    ...
    $quoted_bookings = self::quote_sql_identifier($table_bookings);
    $quoted_services = self::quote_sql_identifier($table_services);

    $result = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT b.service_id, COALESCE(s.name,'(deleted)') as name, COUNT(*) as bookings, COALESCE(SUM(b.total_price),0) as revenue FROM {$quoted_bookings} b LEFT JOIN {$quoted_services} s ON s.id = b.service_id WHERE b.status = %s AND DATE(b.created_at) BETWEEN %s AND %s GROUP BY b.service_id ORDER BY revenue DESC LIMIT %d",
        'confirmed',
        $from,
        $to,
        $limit
  ...
  public static function top_agents(string $from, string $to, int $limit = 5): array {
    ...
    $quoted_bookings = self::quote_sql_identifier($table_bookings);
    $quoted_agents = self::quote_sql_identifier($table_agents);

    $result = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT b.agent_id, COALESCE(a.name,'(deleted)') as name, COUNT(*) as bookings, COALESCE(SUM(b.total_price),0) as revenue FROM {$quoted_bookings} b LEFT JOIN {$quoted_agents} a ON a.id = b.agent_id WHERE b.status = %s AND DATE(b.created_at) BETWEEN %s AND %s GROUP BY b.agent_id ORDER BY revenue DESC LIMIT %d",
        'confirmed',
        $from,
        $to,
        $limit
  ...
  public static function top_categories(string $from, string $to, int $limit = 5): array {
    ...
    $quoted_bookings = self::quote_sql_identifier($table_bookings);
    $quoted_service_categories = self::quote_sql_identifier($table_service_categories);
    $quoted_categories = self::quote_sql_identifier($table_categories);

    $result = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT c.id as category_id, COALESCE(c.name,'(deleted)') as name, COUNT(DISTINCT b.id) as bookings, COALESCE(SUM(b.total_price),0) as revenue FROM {$quoted_bookings} b LEFT JOIN {$quoted_service_categories} m ON m.service_id = b.service_id LEFT JOIN {$quoted_categories} c ON c.id = m.category_id WHERE b.status = %s AND DATE(b.created_at) BETWEEN %s AND %s GROUP BY c.id ORDER BY revenue DESC LIMIT %d",
        'confirmed',
        $from,
        $to,
        $limit
  ...
  public static function pending_bookings(int $limit = 8): array {
    ...
    $result = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT id, customer_name, customer_email, start_date, start_time, total_price, created_at FROM {$quoted_bookings} WHERE status = %s ORDER BY id DESC LIMIT %d",
        'pending',
        $limit
      ),
      ARRAY_A
  ...
  public static function recent_bookings(int $limit = 10): array {
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT id, customer_name, customer_email, status, total_price, created_at, start_date, start_time FROM {$quoted_bookings} ORDER BY id DESC LIMIT %d",
        $limit
      ),
      ARRAY_A
    );
