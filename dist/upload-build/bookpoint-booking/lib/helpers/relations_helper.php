...
final class POINTLYBOOKING_RelationsHelper {
  ...
  public static function migrate_legacy_relations(): array {
    ...
    if (
      !self::is_safe_sql_identifier($t_services) ||
      !self::is_safe_sql_identifier($t_extras) ||
      !self::is_safe_sql_identifier($map_sc) ||
      !self::is_safe_sql_identifier($map_es)
    ) {
      return [
        'services_migrated' => 0,
        'extras_migrated' => 0,
    ...
    $quoted_services = self::quote_sql_identifier($t_services);
    $quoted_extras = self::quote_sql_identifier($t_extras);
    $quoted_map_sc = self::quote_sql_identifier($map_sc);
    $quoted_map_es = self::quote_sql_identifier($map_es);

    $done_sc = 0;
    $done_es = 0;

    $rows = $wpdb->get_results(
      "SELECT id, category_id FROM {$quoted_services} WHERE category_id IS NOT NULL AND category_id > 0",
      ARRAY_A
    );
    foreach ($rows as $r) {
      $sid = (int)$r['id'];
      ...
      $wpdb->query(
        $wpdb->prepare(
          "INSERT IGNORE INTO {$quoted_map_sc} (service_id, category_id) VALUES (%d, %d)",
          $sid,
          $cid
        )
      );
    ...
    $rows2 = $wpdb->get_results(
      "SELECT id, service_id FROM {$quoted_extras} WHERE service_id IS NOT NULL AND service_id > 0",
      ARRAY_A
    );
    foreach ($rows2 as $r) {
      $eid = (int)$r['id'];
      ...
      $wpdb->query(
        $wpdb->prepare(
          "INSERT IGNORE INTO {$quoted_map_es} (extra_id, service_id) VALUES (%d, %d)",
          $eid,
          $sid
        )
      );
  ...
  public static function sync_relations(bool $sync_legacy_columns = true): array {
    ...
    $rows = $wpdb->get_results(
      ...
      LEFT JOIN {$quoted_map_sc} m ON m.service_id = s.id
    ...
    $rows2 = $wpdb->get_results(
      ...
      LEFT JOIN {$quoted_map_es} m ON m.extra_id = e.id
    ...
    if ($sync_legacy_columns) {
      $service_ids = $wpdb->get_col(
        "SELECT DISTINCT service_id FROM {$quoted_map_sc}"
      );
      foreach ($service_ids as $sid_raw) {
        $sid = (int)$sid_raw;
        $cid = (int)$wpdb->get_var(
          $wpdb->prepare(
            "SELECT category_id FROM {$quoted_map_sc} WHERE service_id=%d ORDER BY category_id ASC LIMIT 1",
            $sid
          )
        );
        if ($sid > 0) {
      ...
      $extra_ids = $wpdb->get_col(
        "SELECT DISTINCT extra_id FROM {$quoted_map_es}"
      );
      foreach ($extra_ids as $eid_raw) {
        $eid = (int)$eid_raw;
        $sid = (int)$wpdb->get_var(
          $wpdb->prepare(
            "SELECT service_id FROM {$quoted_map_es} WHERE extra_id=%d ORDER BY service_id ASC LIMIT 1",
            $eid
          )
        );
        if ($eid > 0) {
