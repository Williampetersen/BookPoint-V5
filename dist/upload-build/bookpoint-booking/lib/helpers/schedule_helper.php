...
final class POINTLYBOOKING_ScheduleHelper {
  ...
  public static function get_schedule_settings(): array {
    ...
    if (!self::is_safe_sql_identifier($t)) return $defaults;

    $quoted_table = self::quote_sql_identifier($t);

    $row = $wpdb->get_row(
      $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE id=%d", 1),
      ARRAY_A
    );
    if (!$row) {
      $wpdb->insert($t, [
  ...
  public static function set_schedule_settings(int $slot_interval_minutes, string $timezone): bool {
    ...
    if (!self::is_safe_sql_identifier($t)) return false;

    $slot_interval_minutes = max(5, min(120, $slot_interval_minutes));
    $timezone = trim($timezone) ?: 'Europe/Copenhagen';
    $quoted_table = self::quote_sql_identifier($t);

    $exists = (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$quoted_table} WHERE id=%d", 1));
  ...
  private static function get_schedule_rows(int $agent_id, int $weekday): array {
    ...
    if (!self::is_safe_sql_identifier($t)) return [];
    ...
    if ($agent_id > 0) {
      $rows = $wpdb->get_results(
        $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE agent_id=%d AND day_of_week=%d ORDER BY start_time ASC", $agent_id, $weekday),
        ARRAY_A
      ) ?: [];
    }
    ...
    if (empty($rows)) {
      $rows = $wpdb->get_results(
        $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE agent_id IS NULL AND day_of_week=%d ORDER BY start_time ASC", $weekday),
        ARRAY_A
      ) ?: [];
    }
  ...
  public static function get_service_rules(int $service_id) : array {
    ...
    $cols = $wpdb->get_col("SHOW COLUMNS FROM {$quoted_table}") ?: [];
    ...
    $row = $wpdb->get_row(
      $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE id=%d", $service_id),
      ARRAY_A
    );
