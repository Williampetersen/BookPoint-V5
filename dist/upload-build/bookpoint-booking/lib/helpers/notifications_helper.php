...
final class POINTLYBOOKING_Notifications_Helper {
  ...
  public static function list_workflows(array $filters = []): array {
    ...
    $quoted_action_table = self::quote_sql_identifier($action_table);
    $quoted_log_table = self::quote_sql_identifier($log_table);
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        "SELECT w.*,
          (
            SELECT COUNT(*) FROM {$quoted_action_table} a WHERE a.workflow_id = w.id
          ) as actions_count,
          (
            SELECT MAX(l.created_at) FROM {$quoted_log_table} l WHERE l.workflow_id = w.id
          ) as last_run_at,
          (
            SELECT l.status FROM {$quoted_log_table} l WHERE l.workflow_id = w.id ORDER BY l.id DESC LIMIT 1
          ) as last_run_status
        FROM {$quoted_table} w
  ...
  public static function get_workflow(int $workflow_id): ?array {
    ...
    $quoted_table = self::quote_sql_identifier($table);
    $row = $wpdb->get_row(
      $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE id = %d", $workflow_id),
      ARRAY_A
    );
    if (!$row) {
      return null;
  ...
  public static function create_action(int $workflow_id, array $data): ?array {
    ...
    $sort_order = (int)$wpdb->get_var(
      $wpdb->prepare("SELECT IFNULL(MAX(sort_order), 0) + 1 FROM {$quoted_table} WHERE workflow_id = %d", $workflow_id)
    );
  ...
  public static function get_action(int $action_id): ?array {
    ...
    $quoted_table = self::quote_sql_identifier($table);
    $row = $wpdb->get_row(
      $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE id = %d", $action_id),
      ARRAY_A
    );
    if (!$row) {
      return null;
  ...
  private static function fetch_workflows(string $event_key): array {
    ...
    $quoted_table = self::quote_sql_identifier($table);
    $rows = $wpdb->get_results(
      $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE event_key = %s AND status = 'active' ORDER BY updated_at DESC", $event_key),
      ARRAY_A
    );
    return $rows ?: [];
  }
  ...
  private static function fetch_actions(int $workflow_id): array {
    ...
    $quoted_table = self::quote_sql_identifier($table);
    $rows = $wpdb->get_results(
      $wpdb->prepare("SELECT * FROM {$quoted_table} WHERE workflow_id = %d ORDER BY sort_order ASC", $workflow_id),
      ARRAY_A
    );
    if (!$rows) {
      return [];
