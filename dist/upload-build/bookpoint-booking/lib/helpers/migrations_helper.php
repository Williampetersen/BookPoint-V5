...
final class POINTLYBOOKING_MigrationsHelper {
  ...
  private static function maybe_add_column(string $table, string $column, string $definition): void {
    ...
    if (!$exists) {
      self::run_add_column_query($table_sql, $column_sql, $definition);
    }
  }
  ...
  private static function run_add_column_query(string $table_sql, string $column_sql, string $definition): void {
    global $wpdb;
    switch ($definition) {
      case 'BIGINT UNSIGNED NULL':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} BIGINT UNSIGNED NULL");
        return;
      case 'LONGTEXT NULL':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} LONGTEXT NULL");
        return;
      case 'VARCHAR(60) NULL':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} VARCHAR(60) NULL");
        return;
      case 'DECIMAL(10,2) NOT NULL DEFAULT 0.00':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} DECIMAL(10,2) NOT NULL DEFAULT 0.00");
        return;
      case "CHAR(3) NOT NULL DEFAULT 'USD'":
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} CHAR(3) NOT NULL DEFAULT 'USD'");
        return;
      case "VARCHAR(30) NOT NULL DEFAULT 'cash'":
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} VARCHAR(30) NOT NULL DEFAULT 'cash'");
        return;
      case "VARCHAR(30) NOT NULL DEFAULT 'unpaid'":
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} VARCHAR(30) NOT NULL DEFAULT 'unpaid'");
        return;
      case 'VARCHAR(190) NULL':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} VARCHAR(190) NULL");
        return;
      case 'DECIMAL(10,2) NULL':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} DECIMAL(10,2) NULL");
        return;
      case 'CHAR(3) NULL':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} CHAR(3) NULL");
        return;
      case 'TINYINT(1) NOT NULL DEFAULT 0':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} TINYINT(1) NOT NULL DEFAULT 0");
        return;
      case 'DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP");
        return;
      case 'DATETIME NULL':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} DATETIME NULL");
        return;
      case "VARCHAR(30) NOT NULL DEFAULT 'pending'":
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} VARCHAR(30) NOT NULL DEFAULT 'pending'");
        return;
      case 'INT NOT NULL DEFAULT 0':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} INT NOT NULL DEFAULT 0");
        return;
      case 'INT NOT NULL DEFAULT 1':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} INT NOT NULL DEFAULT 1");
        return;
      case 'BIGINT UNSIGNED NULL AFTER customer_id':
        $wpdb->query("ALTER TABLE {$table_sql} ADD COLUMN {$column_sql} BIGINT UNSIGNED NULL AFTER customer_id");
        return;
    }
  }
