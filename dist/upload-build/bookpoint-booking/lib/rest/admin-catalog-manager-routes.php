...
function pointlybooking_rest_admin_categories_list(WP_REST_Request $req) {
  ...
  $quoted_categories = '`' . str_replace('`', '``', $t) . '`';
  $quoted_rel = '`' . str_replace('`', '``', $t_rel) . '`';
  $rows = $wpdb->get_results(
    "SELECT c.*, COUNT(r.service_id) AS services_count
       FROM {$quoted_categories} c
       LEFT JOIN {$quoted_rel} r ON r.category_id = c.id
       GROUP BY c.id
...
function pointlybooking_rest_admin_extras_list(WP_REST_Request $req) {
  ...
  $rows = $wpdb->get_results(
    "SELECT * FROM {$quoted_table} ORDER BY sort_order ASC, id DESC",
    ARRAY_A
  ) ?: [];
  foreach ($rows as &$r) {
    $r['image_id'] = (int)($r['image_id'] ?? 0);
...
function pointlybooking_rest_admin_agents_list_full(WP_REST_Request $req) {
  ...
  $quoted_agents = '`' . str_replace('`', '``', $t) . '`';
  $quoted_rel = '`' . str_replace('`', '``', $t_rel) . '`';
  $rows = $wpdb->get_results(
    "SELECT a.*, COUNT(r.service_id) AS services_count
       FROM {$quoted_agents} a
       LEFT JOIN {$quoted_rel} r ON r.agent_id = a.id
       GROUP BY a.id
       ORDER BY a.id DESC",
    ARRAY_A
  ) ?: [];
...
function pointlybooking_rest_admin_service_get_categories(WP_REST_Request $req) {
  ...
  $quoted_table = '`' . str_replace('`', '``', $t) . '`';
  $ids = $wpdb->get_col(
    $wpdb->prepare("SELECT category_id FROM {$quoted_table} WHERE service_id=%d", $service_id)
  ) ?: [];
...
function pointlybooking_rest_admin_extra_get_services(WP_REST_Request $req) {
  ...
  $quoted_table = '`' . str_replace('`', '``', $t) . '`';
  $ids = $wpdb->get_col(
    $wpdb->prepare("SELECT service_id FROM {$quoted_table} WHERE extra_id=%d", $extra_id)
  ) ?: [];
  $ids = array_map('intval', $ids);
  return new WP_REST_Response(['status'=>'success','data'=>$ids], 200);
}
