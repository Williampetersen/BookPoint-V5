...
function pointlybooking_rest_admin_customers_list(WP_REST_Request $req) {
  ...
  if (!preg_match('/^[A-Za-z0-9_]+$/', $tCustomers) || !preg_match('/^[A-Za-z0-9_]+$/', $tBookings)) {
  ...
  $quoted_customers = '`' . str_replace('`', '``', $tCustomers) . '`';
  $quoted_bookings = '`' . str_replace('`', '``', $tBookings) . '`';
  ...
  $rows = $wpdb->get_results(
    $wpdb->prepare(
      "SELECT
        c.*,
        (SELECT COUNT(*) FROM {$quoted_bookings} b WHERE b.customer_id = c.id) AS bookings_count
      FROM {$quoted_customers} c
      WHERE (
        %d = 0
  ...
  $total = (int)$wpdb->get_var(
    $wpdb->prepare(
      "SELECT COUNT(*) FROM {$quoted_customers} c
       WHERE (
         %d = 0
         OR c.first_name LIKE %s
         OR c.last_name LIKE %s
...
function pointlybooking_rest_admin_customer_get(WP_REST_Request $req) {
  ...
  $quoted_customers = '`' . str_replace('`', '``', $tCustomers) . '`';
  $quoted_bookings = '`' . str_replace('`', '``', $tBookings) . '`';
  $quoted_services = '`' . str_replace('`', '``', $tServices) . '`';
  $quoted_agents = '`' . str_replace('`', '``', $tAgents) . '`';

  $customer = $wpdb->get_row(
    $wpdb->prepare("SELECT * FROM {$quoted_customers} WHERE id = %d", $id),
    ARRAY_A
  );
...
function pointlybooking_rest_admin_customer_form_fields(WP_REST_Request $req) {
  ...
  $rows = $wpdb->get_results(
    $wpdb->prepare("SELECT * FROM {$quoted_table}
      WHERE scope=%s
...
function pointlybooking_rest_admin_tools_export_settings(WP_REST_Request $req) {
  ...
  $rows = $wpdb->get_results(
    "SELECT setting_key, setting_value FROM {$quoted_table}",
    ARRAY_A
  ) ?: [];
  $settings = [];
  foreach ($rows as $r) {
