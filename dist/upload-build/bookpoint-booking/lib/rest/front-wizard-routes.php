...
function pointlybooking_rest_front_locations() {
  ...
  $rows = $wpdb->get_results(
    "SELECT id,name,address,status,image_id FROM {$quoted_table} WHERE status='active' ORDER BY id DESC",
    ARRAY_A
  ) ?: [];
...
function pointlybooking_front_services_format_rows(array $rows): array {
  ...
  $cols = $wpdb->get_col(
    "SHOW COLUMNS FROM {$quoted_table}"
  ) ?: [];
...
function pointlybooking_rest_front_services(WP_REST_Request $req) {
  ...
  $cols = $wpdb->get_col(
    "SHOW COLUMNS FROM {$quoted_services}"
  ) ?: [];
  ...
  if ($category_ids && $rel_exists) {
    ...
    foreach ($category_ids as $category_id) {
      $matched = $wpdb->get_col(
        $wpdb->prepare("SELECT service_id FROM {$quoted_rel} WHERE category_id=%d", (int)$category_id)
      ) ?: [];
