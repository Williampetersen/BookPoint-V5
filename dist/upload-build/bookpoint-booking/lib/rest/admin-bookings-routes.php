...
function pointlybooking_rest_admin_booking_get(WP_REST_Request $req) {
  ...
  $quoted_bookings = pointlybooking_quote_sql_identifier($tBookings);
  $quoted_customers = pointlybooking_quote_sql_identifier($tCustomers);
  $quoted_services = pointlybooking_quote_sql_identifier($tServices);
  $quoted_agents = pointlybooking_quote_sql_identifier($tAgents);
  $quoted_fields = pointlybooking_quote_sql_identifier($tFields);
  ...
  if ($customer_id && $table_exists($tCustomers)) {
    $cRow = $wpdb->get_row(
      $wpdb->prepare(
        "SELECT * FROM {$quoted_customers} WHERE id=%d",
        $customer_id
      ),
      ARRAY_A
    );
  ...
  if ($service_id && $table_exists($tServices)) {
    $sRow = $wpdb->get_row(
      $wpdb->prepare(
        "SELECT * FROM {$quoted_services} WHERE id=%d",
        $service_id
      ),
      ARRAY_A
    );
  ...
  if ($agent_id && $table_exists($tAgents)) {
    $aRow = $wpdb->get_row(
      $wpdb->prepare(
        "SELECT * FROM {$quoted_agents} WHERE id=%d",
        $agent_id
      ),
      ARRAY_A
    );
    ...
    if (!empty($ids)) {
      ...
      if (pointlybooking_is_safe_sql_identifier($t_extras)) {
        ...
        $rows = $wpdb->get_results(
          $wpdb->prepare(
            "SELECT id, name, price FROM {$quoted_extras} WHERE id IN (" . implode(',', array_fill(0, count($ids), '%d')) . ')',
            $ids
          ),
          ARRAY_A
        ) ?: [];
  ...
  if ($table_exists($tFields)) {
    $defs = $wpdb->get_results("SELECT * FROM {$quoted_fields} ORDER BY sort_order ASC, id ASC", ARRAY_A) ?: [];
    foreach($defs as $d){
      $field_defs[] = [
...
function pointlybooking_rest_admin_agents_list(WP_REST_Request $req) {
  ...
  $quoted_agents = pointlybooking_quote_sql_identifier($t);
  ...
  if ($has_name) {
    $rows = $wpdb->get_results(
      "SELECT * FROM {$quoted_agents} ORDER BY name ASC",
      ARRAY_A
    ) ?: [];
  } elseif ($has_first && $has_last) {
    $rows = $wpdb->get_results(
      "SELECT * FROM {$quoted_agents} ORDER BY first_name ASC, last_name ASC",
      ARRAY_A
    ) ?: [];
  } elseif ($has_first) {
    $rows = $wpdb->get_results(
      "SELECT * FROM {$quoted_agents} ORDER BY first_name ASC",
      ARRAY_A
    ) ?: [];
  } elseif ($has_last) {
    $rows = $wpdb->get_results(
      "SELECT * FROM {$quoted_agents} ORDER BY last_name ASC",
      ARRAY_A
    ) ?: [];
  } else {
    $rows = $wpdb->get_results(
      "SELECT * FROM {$quoted_agents} ORDER BY id ASC",
      ARRAY_A
    ) ?: [];
  }
...
function pointlybooking_rest_admin_booking_patch(WP_REST_Request $req) {
  ...
  if ($start_date !== null || $start_time !== null) {
    ...
    $svc = $wpdb->get_row(
      $wpdb->prepare(
        "SELECT * FROM {$quoted_services} WHERE id=%d LIMIT 1",
        $service_id
      ),
      ARRAY_A
    );
...
function pointlybooking_rest_admin_booking_delete(WP_REST_Request $req) {
  ...
  $row = $wpdb->get_row(
    $wpdb->prepare(
      "SELECT id FROM {$quoted_bookings} WHERE id=%d",
      $id
    ),
    ARRAY_A
  );
...
function pointlybooking_has_conflict($agent_id, $date, $start_hhmm, $end_hhmm, $exclude_booking_id = 0){
  ...
  if (!pointlybooking_is_safe_sql_identifier($tB) || !pointlybooking_is_safe_sql_identifier($tS)) {
    return false;
  }
