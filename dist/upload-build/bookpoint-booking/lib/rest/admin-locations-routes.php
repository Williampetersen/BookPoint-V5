...
function pointlybooking_rest_admin_locations_list() {
  ...
  if (!preg_match('/^[A-Za-z0-9_]+$/', $loc) || !preg_match('/^[A-Za-z0-9_]+$/', $cat)) {
    return rest_ensure_response(['status' => 'success', 'data' => []]);
  }

  $quoted_loc = '`' . str_replace('`', '``', $loc) . '`';
  $quoted_cat = '`' . str_replace('`', '``', $cat) . '`';

  $rows = $wpdb->get_results(
    "SELECT l.*, c.name AS category_name, c.image_id AS category_image_id
       FROM {$quoted_loc} l
       LEFT JOIN {$quoted_cat} c ON c.id = l.category_id
       ORDER BY l.id DESC",
    ARRAY_A
  ) ?: [];
...
function pointlybooking_rest_admin_locations_get($req) {
  ...
  $quoted_loc = '`' . str_replace('`', '``', $loc) . '`';
  $row = $wpdb->get_row(
    $wpdb->prepare("SELECT * FROM {$quoted_loc} WHERE id=%d", $id),
    ARRAY_A
  );
...
function pointlybooking_rest_admin_locations_agents_get(WP_REST_Request $req) {
  ...
  $quoted_map = '`' . str_replace('`', '``', $map) . '`';
  $rows = $wpdb->get_results(
    $wpdb->prepare("SELECT * FROM {$quoted_map} WHERE location_id=%d", $id),
    ARRAY_A
  ) ?: [];
...
function pointlybooking_rest_admin_location_categories_list() {
  ...
  $rows = $wpdb->get_results(
    "SELECT * FROM {$quoted_table} WHERE status='active' ORDER BY id DESC",
    ARRAY_A
  ) ?: [];
