  ...
  if ((string)$wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $t)) === $t) {
    if ($agent_id > 0) {
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          "SELECT * FROM {$quoted_table} WHERE agent_id=%d ORDER BY day_of_week ASC, start_time ASC",
          $agent_id
        ),
        ARRAY_A
      ) ?: [];
    ...
    if (empty($rows)) {
      $rows = $wpdb->get_results(
        "SELECT * FROM {$quoted_table} WHERE agent_id IS NULL ORDER BY day_of_week ASC, start_time ASC",
        ARRAY_A
      ) ?: [];
    }
  }
...
function pointlybooking_rest_admin_schedule_save(WP_REST_Request $req) {
  ...
  if ($agent_id > 0) {
    $wpdb->query(
      $wpdb->prepare("DELETE FROM {$quoted_table} WHERE agent_id=%d", $agent_id)
    );
  } else {
    $wpdb->query(
      "DELETE FROM {$quoted_table} WHERE agent_id IS NULL"
    );
  }
