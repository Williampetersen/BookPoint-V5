...
function pointlybooking_rest_admin_get_agent_schedule(WP_REST_Request $req) {
  ...
  $quoted_hours = '`' . str_replace('`', '``', $t_hours) . '`';
  $quoted_breaks = '`' . str_replace('`', '``', $t_breaks) . '`';
...
function pointlybooking_rest_admin_save_agent_schedule(WP_REST_Request $req) {
  ...
  $wpdb->query(
    $wpdb->prepare("DELETE FROM {$quoted_hours} WHERE agent_id=%d", $agent_id)
  );
  $wpdb->query(
    $wpdb->prepare("DELETE FROM {$quoted_breaks} WHERE agent_id=%d", $agent_id)
  );
