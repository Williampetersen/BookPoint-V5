...
function pointlybooking_rest_front_booking_access(WP_REST_Request $req) {
  ...
  $stored_key = $wpdb->get_var(
    $wpdb->prepare("SELECT manage_key FROM {$quoted_table} WHERE id=%d", $id)
  );
...
add_action('rest_api_init', function () {
  register_rest_route('pointly-booking/v1', '/front/bookings/(?P<id>\d+)/status', [
    ...
    'callback' => function (WP_REST_Request $req) {
      ...
      $row = $wpdb->get_row(
        $wpdb->prepare(
          "SELECT id, status, payment_method, payment_status, payment_provider_ref FROM {$quoted_table} WHERE id=%d",
          $id
        ),
        ARRAY_A
      );
  ...
  register_rest_route('pointly-booking/v1', '/front/bookings/(?P<id>\d+)/payment-cancel', [
    ...
    'callback' => function (WP_REST_Request $req) {
      ...
      $quoted_table = '`' . str_replace('`', '``', $table) . '`';
      $row = $wpdb->get_row(
        $wpdb->prepare("SELECT id, payment_status FROM {$quoted_table} WHERE id=%d", $id),
        ARRAY_A
      );
      if (!$row) {
        return new WP_Error('not_found', 'Booking not found', ['status' => 404]);
