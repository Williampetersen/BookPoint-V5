...
function pointlybooking_rest_admin_schedule_get(WP_REST_Request $req) {
  ...
  if ((string)$wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $t)) === $t) {
    if ($agent_id > 0) {
      $rows = $wpdb->get_results(
        pointlybooking_prepare_query_with_identifiers(
          "SELECT * FROM %i WHERE agent_id=%d ORDER BY day_of_week ASC, start_time ASC",
          [$t],
          [$agent_id]
        ),
        ARRAY_A
    ...
    if (empty($rows)) {
      $rows = $wpdb->get_results(
        pointlybooking_prepare_query_with_identifiers(
          "SELECT * FROM %i WHERE agent_id IS NULL ORDER BY day_of_week ASC, start_time ASC",
          [$t]
        ),
        ARRAY_A
      ) ?: [];
...
function pointlybooking_rest_admin_schedule_save(WP_REST_Request $req) {
  ...
  if ($agent_id > 0) {
    $wpdb->query(
      pointlybooking_prepare_query_with_identifiers(
        "DELETE FROM %i WHERE agent_id=%d",
        [$t],
        [$agent_id]
      )
    );
    ...
    $wpdb->query(
      pointlybooking_prepare_query_with_identifiers(
        "DELETE FROM %i WHERE agent_id IS NULL",
        [$t]
      )
    );
  }
