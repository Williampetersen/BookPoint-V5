...
function pointlybooking_generate_slots_for_public(string $date, int $agent_id, int $service_id, int $occupied_min, int $capacity, array &$debug_meta = []): array {
  ...
  foreach ($windows as $w) {
    ...
    if ($capacity <= 1) {
      $conflict = (int)$wpdb->get_var(pointlybooking_prepare_query_with_identifiers("
        SELECT COUNT(*)
        FROM %i b
        ...
        LIMIT 1
      ", [$t_book, $t_srv], [$agent_id, $date, $date, $end_time, $date, $start_time]));

      if ($conflict > 0) continue;
    } else {
      $overlapCount = (int)$wpdb->get_var(pointlybooking_prepare_query_with_identifiers("
        SELECT COUNT(*)
        FROM %i b
        LEFT JOIN %i s ON s.id=b.service_id
        WHERE b.agent_id=%d
