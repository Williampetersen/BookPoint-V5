...
<?php
  $actions_html = '<a class="bp-top-btn" href="' . esc_url(admin_url('admin.php?page=pointlybooking_categories_edit')) . '">' . esc_html__('Add New', 'bookpoint') . '</a>';
  pointlybooking_render_legacy_shell_start(esc_html__('Categories', 'bookpoint'), esc_html__('Organize your services into categories.', 'bookpoint'), $actions_html, 'categories');
?>
  ...
  <table class="widefat striped">
    ...
    <tbody>
      ...
      <?php else: foreach ($items as $row):
        ...
        if (!$img) {
          $initial = mb_substr((string)($row['name'] ?? 'Ã¢â‚¬â€'), 0, 1);
        ...
        <tr>
          ...
          <td>
            <strong>
              <a href="<?php echo esc_url(admin_url('admin.php?page=pointlybooking_categories&action=edit&id='.(int)$row['id'])); ?>">
          ...
          <td>
            <?php echo ((int)$row['is_active'] === 1) ? 'Ã¢Å“â€¦ ' . esc_html__('Active', 'bookpoint') : 'Ã¢â‚¬â€ ' . esc_html__('Inactive', 'bookpoint'); ?>
          </td>
          <td><?php echo (int)$row['sort_order']; ?></td>
          <td>
            <a class="button button-small" href="<?php echo esc_url(admin_url('admin.php?page=pointlybooking_categories_edit&id='.(int)$row['id'])); ?>">
