...
<?php
  $actions_html = '<a class="bp-top-btn" href="' . esc_url(admin_url('admin.php?page=pointlybooking_extras&action=edit')) . '">' . esc_html__('Add New', 'bookpoint') . '</a>';
  pointlybooking_render_legacy_shell_start(esc_html__('Service Extras', 'bookpoint'), esc_html__('Manage add-ons attached to services.', 'bookpoint'), $actions_html, 'extras');
?>
    ...
    <tbody>
      ...
      <?php else: foreach ($items as $row):
        ...
        <tr>
          ...
          <td>
            <strong>
              <a href="<?php echo esc_url(admin_url('admin.php?page=pointlybooking_extras&action=edit&id='.(int)$row['id'])); ?>">
                <?php echo esc_html($row['name']); ?>
              </a>
            </strong>
            <div style="color:#666;"><?php echo esc_html(wp_trim_words((string)$row['description'], 14)); ?></div>
          </td>
          <td><?php echo esc_html($row['service_name'] ?? 'Ã¢â‚¬â€'); ?></td>
          <td><?php echo esc_html(number_format((float)$row['price'], 2)); ?></td>
          <td><?php echo !empty($row['duration_min']) ? esc_html((string)(int)$row['duration_min'].' min') : 'Ã¢â‚¬â€'; ?></td>
          <td><?php echo ((int)$row['is_active'] === 1) ? 'Ã¢Å“â€¦ ' . esc_html__('Active', 'bookpoint') : 'Ã¢â‚¬â€ ' . esc_html__('Inactive', 'bookpoint'); ?></td>
          <td>
            <a class="button button-small" href="<?php echo esc_url(admin_url('admin.php?page=pointlybooking_extras&action=edit&id='.(int)$row['id'])); ?>">
