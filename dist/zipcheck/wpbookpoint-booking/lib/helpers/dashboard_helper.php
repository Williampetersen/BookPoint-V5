...
final class POINTLYBOOKING_DashboardHelper {
  ...
  public static function range(): array {
    $range = sanitize_text_field(wp_unslash($_GET['range'] ?? '14'));
    ...
    if ($range === 'custom') {
      ...
      if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $from)) $from = gmdate('Y-m-d', strtotime('-13 days', strtotime($today)));
  ...
  public static function top_categories(string $from, string $to, int $limit = 5): array {
    ...
    $m = $wpdb->prefix . 'pointlybooking_service_categories';
    $c = $wpdb->prefix . 'pointlybooking_categories';
