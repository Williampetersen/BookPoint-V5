...
final class POINTLYBOOKING_AdminSettingsController extends POINTLYBOOKING_Controller {
  ...
  public function save_license(): void {
    $this->require_cap('pointlybooking_manage_settings');
    check_admin_referer('pointlybooking_admin');
    wp_safe_redirect(admin_url('admin.php?page=pointlybooking_settings&tab=general'));
    exit;
  }
  ...
  public function import_json(): void {
    ...
    if (empty($_FILES['pointlybooking_settings_file']['tmp_name'])) {
      wp_safe_redirect(admin_url('admin.php?page=pointlybooking_settings&tab=import_export&import=missing'));
      exit;
    }

    $raw = file_get_contents($_FILES['pointlybooking_settings_file']['tmp_name']);
    $data = json_decode($raw, true);

    if (!is_array($data) || ($data['plugin'] ?? '') !== 'bookpoint') {
      wp_safe_redirect(admin_url('admin.php?page=pointlybooking_settings&tab=import_export&import=badfile'));
      exit;
    }

    $settings = $data['pointlybooking_settings'] ?? null;
    if (!is_array($settings)) {
      wp_safe_redirect(admin_url('admin.php?page=pointlybooking_settings&tab=import_export&import=badsettings'));
      exit;
    }
    ...
    wp_safe_redirect(admin_url('admin.php?page=pointlybooking_settings&tab=import_export&import=ok'));
    exit;
  }
