...
if (!function_exists('pointlybooking_request_raw_value')) {
  function pointlybooking_request_raw_value(string $method, string $key) {
    if ($key === '') {
      return null;
    }

    if ($method === 'get') {
      if (!isset($_GET[$key])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing -- Centralized low-level accessor only; nonce/capability checks happen in action handlers.
        return null;
      }
      return wp_unslash($_GET[$key]); // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Low-level accessor returns unslashed raw input for typed wrapper handling.
    }

    if ($method === 'post') {
      if (!isset($_POST[$key])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing -- Centralized low-level accessor only; nonce/capability checks happen in action handlers.
        return null;
      }
      return wp_unslash($_POST[$key]); // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Low-level accessor returns unslashed raw input for typed wrapper handling.
    }
    ...
    if ($method === 'server') {
      ...
      return wp_unslash($_SERVER[$key]); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Low-level accessor returns unslashed raw input for typed wrapper handling.
    }
...
if (!function_exists('pointlybooking_request_has_key')) {
  function pointlybooking_request_has_key(string $method, string $key): bool {
    return pointlybooking_request_raw_value($method, $key) !== null;
  }
}

if (!function_exists('pointlybooking_request_value_is_array')) {
  function pointlybooking_request_value_is_array(string $method, string $key): bool {
    return is_array(pointlybooking_request_raw_value($method, $key));
  }
}

if (!function_exists('pointlybooking_request_array')) {
  function pointlybooking_request_array(string $method, string $key): array {
    $value = pointlybooking_request_raw_value($method, $key);
    if (!is_array($value)) {
      return [];
    }
    return $value;
...
if (!function_exists('pointlybooking_request_scalar')) {
  function pointlybooking_request_scalar(string $method, string $key): string {
    $value = pointlybooking_request_raw_value($method, $key);
...
if ($pointlybooking_existing_class !== '') {
  ...
  if (!$pointlybooking_same_file) {
    ...
    if ($pointlybooking_existing_file !== '') {
      ...
      $pointlybooking_loaded_from = __('Loaded from: %s', 'bookpoint-v5');
    ...
    add_action('admin_notices', function () use ($pointlybooking_product, $pointlybooking_details) {
      ...
      $message_template = __('%1$s could not be loaded because another copy of BookPoint is already active. Please deactivate the other BookPoint plugin first, then activate %2$s.%3$s', 'bookpoint-v5');
      $message = sprintf(
        $message_template,
...
add_filter('admin_body_class', function($classes){
  ...
  $page = sanitize_key(pointlybooking_request_scalar('get', 'page'));
...
final class POINTLYBOOKING_Core_Plugin {
  ...
  private static function register_hooks() : void {
    ...
    add_action('admin_post_pointlybooking_admin_booking_quick_update', function () {
      ...
      $id = absint(pointlybooking_request_scalar('post', 'id'));
      $status = sanitize_key(pointlybooking_request_scalar('post', 'status'));
  ...
  public static function plugin_action_links(array $links): array {
    ...
    $custom = [
      '<a href="' . esc_url($dash) . '">' . esc_html__('Open BookPoint', 'bookpoint-v5') . '</a>',
      '<a href="' . esc_url($settings) . '">' . esc_html__('Settings', 'bookpoint-v5') . '</a>',
    ];
  ...
  private static function seed_default_agent_hours() : void {
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct bootstrap reads use validated plugin table names and should remain uncached.
    $agents = $wpdb->get_results(
      // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this bootstrap query.
      "SELECT id FROM {$t_agents} ORDER BY id ASC",
      ARRAY_A
    ) ?: [];
    foreach ($agents as $a) {
      $aid = (int)$a['id'];
      ...
      $exists = (int) $wpdb->get_var(
        $wpdb->prepare(
          // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this bootstrap query.
          "SELECT COUNT(*) FROM {$t_hours} WHERE agent_id=%d",
          $aid
        )
      );
  ...
  public static function register_rest_routes() : void {
    ...
    register_rest_route('pointly-booking/v1', '/admin/bookings', [
      ...
      'callback' => function(\WP_REST_Request $req){
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct admin reads use validated plugin table names and should remain uncached.
        $total = (int) $wpdb->get_var(
          $wpdb->prepare(
            // phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifiers are required in this admin bookings count query.
            "SELECT COUNT(*)
        ...
        if ($order === 'ASC') {
          ...
          $items = $wpdb->get_results(
            $wpdb->prepare(
              // phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifiers are required in this admin bookings list query.
              "SELECT
                 b.id,
                 b.start_datetime,
                 b.end_datetime,
    ...
    register_rest_route('pointly-booking/v1', '/admin/bookings/(?P<id>\d+)', [
      ...
      'callback' => function(\WP_REST_Request $req){
        ...
        $services_table = $wpdb->prefix . 'pointlybooking_services';
        $agents_table = $wpdb->prefix . 'pointlybooking_agents';
        $fields_table = $wpdb->prefix . 'pointlybooking_form_fields';
        if (
          !self::is_safe_sql_identifier($bookings_table)
          || !self::is_safe_sql_identifier($customers_table)
          || !self::is_safe_sql_identifier($services_table)
          || !self::is_safe_sql_identifier($agents_table)
          || !self::is_safe_sql_identifier($fields_table)
        ) {
          return new \WP_REST_Response(['status' => 'error', 'message' => 'Invalid table configuration'], 500);
        }
        ...
        $row = $wpdb->get_row(
          $wpdb->prepare(
            // phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifiers are required in this booking detail query.
            "SELECT
               b.*,
               CONCAT(cust.first_name, ' ', cust.last_name) as customer_name,
               cust.email as customer_email,
        ...
        if ($has_form_fields) {
          // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct admin reads use a validated plugin table name and should remain uncached.
          $defs = $wpdb->get_results(
            $wpdb->prepare(
              // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this form field definition query.
  ...
  public static function register_admin_menu() : void {
    ...
    add_submenu_page(
      'pointlybooking_dashboard',
      __('Service Extras', 'bookpoint-v5'),
      __('Service Extras', 'bookpoint-v5'),
      $cap('pointlybooking_manage_services'),
    ...
    add_submenu_page(
      'pointlybooking_dashboard',
      __('Booking Form Designer', 'bookpoint-v5'),
      __('Booking Form Designer', 'bookpoint-v5'),
      $cap('pointlybooking_manage_settings'),
    ...
    add_submenu_page(
      'tools.php',
      __('BookPoint Tools', 'bookpoint-v5'),
      __('BookPoint Tools', 'bookpoint-v5'),
      $cap('pointlybooking_manage_tools'),
      'pointlybooking_tools',
      'pointlybooking_render_admin_app'
    ...
    // Hidden pages for internal use
      ...
      add_submenu_page(
        null,
        __('Edit Location Category', 'bookpoint-v5'),
        __('Edit Location Category', 'bookpoint-v5'),
        'pointlybooking_manage_settings',
    ...
    add_submenu_page(
      null,
      __('Delete Category', 'bookpoint-v5'),
      __('Delete Category', 'bookpoint-v5'),
      'pointlybooking_manage_services',
    ...
    add_submenu_page(
      null,
      __('Delete Service', 'bookpoint-v5'),
      __('Delete Service', 'bookpoint-v5'),
      'pointlybooking_manage_services',
    ...
    add_submenu_page(
      null,
      __('Edit Promo Code', 'bookpoint-v5'),
      __('Edit Promo Code', 'bookpoint-v5'),
      'pointlybooking_manage_settings',
    ...
    add_submenu_page(
      null,
      __('Delete Promo Code', 'bookpoint-v5'),
      __('Delete Promo Code', 'bookpoint-v5'),
      'pointlybooking_manage_settings',
    ...
    add_submenu_page(
      null,
      __('Confirm Booking', 'bookpoint-v5'),
      __('Confirm Booking', 'bookpoint-v5'),
    ...
    add_submenu_page(
      null,
      __('Cancel Booking', 'bookpoint-v5'),
      __('Cancel Booking', 'bookpoint-v5'),
      'pointlybooking_manage_bookings',
  ...
  private static function is_menu_debug_enabled(): bool {
    ...
    return absint(pointlybooking_request_scalar('get', 'pointlybooking_menu_debug')) === 1;
  }
  ...
  public static function enqueue_admin_assets(string $hook): void {
    ...
    $action_param = sanitize_key(pointlybooking_request_scalar('get', 'action'));
  ...
  public static function handle_customers_export_csv() : void {
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct export reads use a validated plugin table name and should remain uncached.
    $rows = $wpdb->get_results(
      // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this export query.
  ...
  public static function shortcode_customer_portal() : string {
    ...
    $step = sanitize_key(pointlybooking_request_scalar('get', 'step'));
    if ($step === '') {
      $step = 'email';
    }
    $email = sanitize_email(pointlybooking_request_scalar('get', 'email'));
    $session = sanitize_text_field(pointlybooking_request_scalar('get', 's'));
    $view_nonce = sanitize_text_field(pointlybooking_request_scalar('get', 'bpv'));
  ...
  public static function handle_portal_posts() : void {
    $action = sanitize_key(pointlybooking_request_scalar('post', 'pointlybooking_portal_action'));
    if ($action === '') return;

    self::rate_limit_or_block('portal_action', 20, 600);

    if ($action === 'send_otp') {
      $nonce = sanitize_text_field(pointlybooking_request_scalar('post', '_wpnonce'));
      if (!wp_verify_nonce($nonce, 'pointlybooking_portal_email')) return;

      $base = self::portal_base_url();
      $email = sanitize_email(pointlybooking_request_scalar('post', 'pointlybooking_portal_email'));
    ...
    if ($action === 'verify_otp') {
      ...
      $otp_raw = sanitize_text_field(pointlybooking_request_scalar('post', 'pointlybooking_portal_otp'));
  ...
  public static function rate_limit_or_block(string $key, int $limit = 30, int $window_sec = 600) : void {
    ...
    if ((int)$data['count'] > $limit) {
      wp_die(esc_html__('Too many requests. Please try again later.', 'bookpoint-v5'), 429);
    }
  }
}
}
...
if (!function_exists('pointlybooking_rest_get_services')) {
  function pointlybooking_rest_get_services(\WP_REST_Request $req) {
    ...
    if (!pointlybooking_is_safe_sql_identifier($services_table) || !pointlybooking_is_safe_sql_identifier($service_categories_table)) {
      return rest_ensure_response(['status' => 'success', 'data' => []]);
    }

    if ($category_id > 0) {
      // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct public reads use validated plugin table names and should remain uncached.
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          // phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifiers are required in this services lookup.
      ...
      // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct public reads use a validated plugin table name and should remain uncached.
      $rows = $wpdb->get_results(
        $wpdb->prepare(
          // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this services lookup.
...
if (!function_exists('pointlybooking_rest_get_extras')) {
  function pointlybooking_rest_get_extras(\WP_REST_Request $req) {
    ...
    if (!pointlybooking_is_safe_sql_identifier($extras_table) || !pointlybooking_is_safe_sql_identifier($extra_services_table)) {
      return rest_ensure_response(['status' => 'success', 'data' => []]);
    }
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        // phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifiers are required in this extras lookup.
...
if (!function_exists('pointlybooking_rest_get_agents')) {
  function pointlybooking_rest_get_agents(\WP_REST_Request $req) {
    ...
    $agents_table = $wpdb->prefix . 'pointlybooking_agents';
    if (!pointlybooking_is_safe_sql_identifier($agent_services_table) || !pointlybooking_is_safe_sql_identifier($agents_table)) {
      return rest_ensure_response(['status'=>'success','data'=>[]]);
    }
    ...
    $rows = $wpdb->get_results(
      $wpdb->prepare(
        // phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifiers are required in this agents lookup.
...
if (!function_exists('pointlybooking_rest_create_booking')) {
  function pointlybooking_rest_create_booking(\WP_REST_Request $req) {
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct booking reads use validated plugin table names and should remain uncached.
    $svc = $wpdb->get_row(
      $wpdb->prepare(
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this service lookup.
    ...
    if ($agent_id > 0) {
      $agent_services_table = $wpdb->prefix . 'pointlybooking_agent_services';
      if (!pointlybooking_is_safe_sql_identifier($agent_services_table)) {
        return rest_ensure_response(['status'=>'error','message'=>'Invalid service configuration']);
      }
      // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct booking reads use a validated plugin table name and should remain uncached.
      $ok = (int) $wpdb->get_var(
        $wpdb->prepare(
          // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this agent-service lookup.
    ...
    $extras_table = $wpdb->prefix . 'pointlybooking_service_extras';
    if (!pointlybooking_is_safe_sql_identifier($extras_table)) {
      return rest_ensure_response(['status'=>'error','message'=>'Invalid service configuration']);
    }
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Direct booking reads use a validated plugin table name and should remain uncached.
    $valid_extras = $wpdb->get_results(
      $wpdb->prepare(
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Validated plugin table identifier is required in this extras price lookup.
